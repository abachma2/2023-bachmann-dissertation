...
once varying all seven variables to prevent unwanted combinations of the 
...
across the input space 4500 times because the Dakota user's manual \cite{adams_dakota_2019}
suggests running 
...
metrics. The Xe-100 discharge burnup was restricted to the specified values 
used in the other sensitivity analysis while the other variables were only restricted 
to the specified range. We randomly sampled (using the Latin Hypercube Sampling 
method in Dakota) each variable based on the possible values and calculated the 
metrics for these samples. This data was then used to create the surrogate model 
to perform the global sensitivity analysis and calculate the Sobol' indices. 


When running the surrogate model we once again used random sampling for all 
of the variables, but allowed all of the input parameters to be continuos variables. 
...
on the surrogate model so that we could calculate the Sobol' indices. The 
...
I actually made MMR burnup a continuos variable for this section of work
