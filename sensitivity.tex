...
across the input space 1500 times. The Xe-100 and \gls{MMR} discharge burnups were 
restricted to the specified values while the other variables were only restricted 
...
We once again used random sampling for all of the variables, but allowed the discharge 
burnups to vary across the range of values instead of restricting them to 
the specific values. We also instructed Dakota to perform variance decomposition 
...
impact of each input parameter on each output metric. 
